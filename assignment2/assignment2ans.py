...
def is_sink(m: List[List[int]], c: List[int]) -> bool:
    ...
    # Cell doesn't exit in m
    if max(c) > (len(m) - 1) or min(c) < 0:
        return False
    ...
    for i in range(-1, 2):
        for j in range(-1, 2):
            ...
            # return False once a neighbour is smaller than the cell
            if m[c[0]][c[1]] > m[i + c[0]][j + c[1]]:
                return False
    return True
...
def find_local_sink(m: List[List[int]], start: List[int]) -> List[int]:
    ...
    def find_lowest_cell_in_neighbours(m, c):
        mapLength = len(m)
        lowestCell = [c[0], c[1]]
        for i in range(-1, 2):
            for j in range(-1, 2):
    ...
    while True:
        lowestCell = find_lowest_cell_in_neighbours(m, start)
        if lowestCell == start:
            return start
        start = lowestCell


def can_hike_to(m: List[List[int]], s: L[int], d: List[int], supplies: int) -> bool:
    """
    ...
    while currentPosition != d:
        # choose East if destination is directly to the East of it OR it costs less or the same as going South
        if currentPosition[0] == d[0] or abs(m[currentPosition[0]][currentPosition[1] + 1] - m[currentPosition[0]][
            currentPosition[1]]) <= abs(
                    m[currentPosition[0] + 1][currentPosition[1]] - m[currentPosition[0]][currentPosition[1]]):
            # choose East
            supplies -= abs(m[currentPosition[0]][currentPosition[1] + 1] - m[currentPosition[0]][currentPosition[1]])
            currentPosition[1] += 1
        else:
            # choose West
            supplies -= abs(m[currentPosition[0] + 1][currentPosition[1]] - m[currentPosition[0]][currentPosition[1]])
            currentPosition[0] += 1
